...
-- 2
SELECT s1.vardas, s1.pavarde, s2.vardas, s2.pavarde, a.isbn, b.isbn
...
WHERE s1.vardas != s2.vardas AND s1.pavarde != s2.pavarde;
...
HAVING count(a.paimta) > (SELECT avg(quantity)
                    FROM (
                        SELECT a.paimta, count(a.paimta) as quantity
                        FROM stud.egzempliorius a
